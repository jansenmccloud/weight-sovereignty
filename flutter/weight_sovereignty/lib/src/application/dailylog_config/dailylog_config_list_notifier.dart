...
final dailyLogConfigListProvider =
    AsyncNotifierProvider<DailyLogConfigListNotifier, List<DailyLogConfig>>(
  DailyLogConfigListNotifier.new,
);

class DailyLogConfigListNotifier extends AsyncNotifier<List<DailyLogConfig>>
    with CrudListNotifierMixin<DailyLogConfig> {
  @override
  CrudRepository<DailyLogConfig> repository() =>
      ref.read(dailyLogConfigRepositoryProvider);

  Future<DailyLogConfig?> getByName(String name) =>
      ref.read(dailyLogConfigRepositoryProvider).getByName(name);
}
